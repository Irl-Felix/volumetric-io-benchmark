...
static void warmup_page_cache(const char *path, uint8_t *buf) {
...
int main() {
    ...
    for (int i = 0; i < n_runs; i++) {
        ...
        ssize_t n;
        while ((n = read(fd, buf, READ_BLOCK)) > 0) {
            /* light-touch to keep a dependency on the bytes without dominating runtime */
            for (ssize_t p = 0; p < n; p += 4096)
                checksum += buf[p];
        }
    ...
    printf("Condition:    C + read  |  warm cache\n");
    printf("Runs:         %d\n", n_runs);
