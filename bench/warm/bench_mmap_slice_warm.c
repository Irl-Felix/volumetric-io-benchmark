...
int main() {
    ...
    for (int r = 0; r < n_runs; r++) {
        ...
        for (int s = 0; s < n_slices; s++) {
            size_t offset  = data_offset + (size_t)s * slice_bytes;
            uint8_t *slice = (uint8_t *)map + offset;
            for (size_t p = 0; p < slice_bytes; p += 4096)
                checksum += slice[p];
        }
        ...
        munmap(map, filesize);
        fprintf(stderr, "run %d checksum: %lu\n", r + 1, checksum);
    }
    ...
    printf("Condition:    C + mmap  |  warm cache  |  slice-by-slice\n");
    printf("Slices:       %d\n", n_slices);
    printf("Runs:         %d\n", n_runs);
